...
<div class="h-[calc(100vh-160px)] flex flex-col md:flex-row gap-6">
    <div class="w-full md:w-80 bg-surface border border-slate-800 rounded-[32px] flex flex-col overflow-hidden">
        <div class="p-6 border-b border-slate-800"><h2 class="text-xl font-black italic uppercase">Чаты</h2></div>
        <div class="flex-1 overflow-y-auto">
            @foreach($this->chats as $chat)
                <button wire:click="selectChat({{ $chat->id }})" 
                    ...
                    <div class="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-xs border border-slate-700">👤</div>
                    <div class="text-left truncate">
                        <p class="font-bold text-sm text-white">{{ $chat->name }}</p>
    ...
    <div class="flex-1 bg-surface border border-slate-800 rounded-[32px] flex flex-col overflow-hidden relative">
        @if($activeChatId)
            <div class="flex-1 overflow-y-auto p-6 space-y-4">
                @foreach($this->messages as $msg)
                    <div class="flex {{ $msg->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-[75%] px-4 py-2 rounded-2xl {{ $msg->sender_id === auth()->id() ? 'bg-brand text-white rounded-tr-none shadow-lg shadow-brand/20' : 'bg-slate-800 text-slate-300 rounded-tl-none border border-slate-700' }}">
                            <p class="text-sm">{{ $msg->body }}</p>
                        </div>
                    </div>
                @endforeach
            </div>
            <form wire:submit="send" class="p-4 bg-bg/40 border-t border-slate-800 flex gap-2">
                <input wire:model="body" type="text" placeholder="Ваше сообщение..." class="flex-1 bg-bg border-slate-700 rounded-xl px-4 py-2">
                <button type="submit" class="bg-brand px-6 py-2 rounded-xl font-black text-xs uppercase italic text-white">ОТПРАВИТЬ</button>
            </form>
        @else
            <div class="flex-1 flex flex-col items-center justify-center text-slate-600 italic">
                <span class="text-6xl mb-4 opacity-20">💬</span>
                <p class="uppercase tracking-widest font-black text-[10px]">Выберите чат для начала общения</p>
            </div>
        @endif
    </div>
</div>